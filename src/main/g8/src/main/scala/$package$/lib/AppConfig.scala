...
import $package$.config.Site
import $package$.lib.util.SnippetHelpers

object AppConfig extends Factory {
  // AuthUserMeta object
  val authUserMeta = new FactoryMaker[AuthUserMeta[_]]($package$.model.User) {}
  ...
  // site settings
  val siteName = new FactoryMaker[String]("$name$") {}
  val systemEmail = new FactoryMaker[String]("$admin_email$") {}
  val systemUsername = new FactoryMaker[String]("$name$ Staff") {}
  ...
  val mailerFromAddrName = new FactoryMaker[Box[String]](Full("$name$ Example Application")) {}
